...
export function useChatSocket({ conversationId, enabled, onMessage }: UseChatSocketOptions) {
  ...
  useEffect(() => {
    ...
    async function connect() {
      setState('connecting')
      try {
        // The value is generated by Cognito for the authenticated session.
        // Environment variables configure only the endpoint and query parameter name.
        const token = await getAccessToken()
        if (!token) throw new Error('A sessão do Cognito não possui um access token válido.')
        const url = new URL(appConfig.websocket.url)
        url.searchParams.set(appConfig.websocket.tokenQueryParam, token)
